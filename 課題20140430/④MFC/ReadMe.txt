================================================================================
    MFC (MICROSOFT FOUNDATION CLASS) ライブラリ : FileControllSamp プロジェクトの概要
===============================================================================

この FileControllSamp アプリケーションは、アプリケーション ウィザードによって作成されました。このアプリケーションは MFC の基本的な使い方を示すだけでなく、アプリケーションを作成するための開始点でもあります。

このファイルには、FileControllSamp アプリケーションを構成する各ファイルの内容の概要が含まれています。

FileControllSamp.vcxproj
...
FileControllSamp.vcxproj.filters
...
FileControllSamp.h
    ...
    その他のプロジェクト固有のヘッダー (Resource.h など) が含まれ、CFileControllSampApp アプリケーション クラスを宣言します。

FileControllSamp.cpp
    これはメインのアプリケーション ソース ファイルで、CFileControllSampApp アプリケーション クラスが含まれています。
...
res\FileControllSamp.ico
    これはアプリケーションのアイコンとして使用されるアイコン ファイルです。このアイコンは、メイン リソース ファイル FileControllSamp.rc にインクルードされます。

res\FileControllSamp.rc2
...
FileControllSampDlg.h, FileControllSampDlg.cpp - ダイアログ
    これらのファイルには、CFileControllSampDlg クラスが含まれます。このクラスはアプリケーションのメイン ダイアログの動作を定義します。ダイアログのテンプレートは、Microsoft Visual C++ で編集できる FileControllSamp.rc に含まれています。
...
StdAfx.h, StdAfx.cpp
    これらのファイルは、FileControllSamp.pch という名前のプリコンパイル済みヘッダー (PCH) ファイルと、StdAfx.obj という名前のプリコンパイル済みの型ファイルをビルドするために使用されます。

Resource.h
    これは、新しいリソース ID を定義する標準のヘッダー ファイルです。このファイルの読み込みおよび更新は、Microsoft Visual C++ で行います。

FileControllSamp.manifest
